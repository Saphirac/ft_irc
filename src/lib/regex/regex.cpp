...
// for all possible ASCII characters
 ...
 * - `type`: A bitmask indicating the type of command (e.g., TYPE_OPEN, TYPE_CLOSE, TYPE_SUFFIX, TYPE_PREFIX,
 * TYPE_CHAR).
 ...
 * The function assumes the string represents a non-negative integer and does not handle negative signs or non-numeric
 * characters.
 *
...
{
	int resI = 0;
	for (; isDigit(*str); str++) resI = resI * 10 + (str[0] - '0');
	return resI;
}
...
void init_regex_mechanism_private(void)
{
	...
	for (int i = 0; i < 128; i++) get_cmd_byChar[i] = end;

	for (; cmd < end; cmd++) get_cmd_byChar[static_cast<unsigned char>(cmd->id)] = cmd;
 ...
 *
 * Note: This function is part of a larger regex processing system and is essential for handling complex patterns with
 * nested structures.
 *
 * Example Usage:
 ...
 */
// Given an expression starting with left parentheses 'p' return a pointer after the end of this expression (right
// parentheses 'rp'). Example: given p=[a*(1+3)[z@][aa]]tp5 returns pointer to: 'tp5' by skipping
//          the [a*(1+3)[z@][aa]] expression.
// Note: handles nested parentheses by remembring the parentheses depth. Example: (((X)Y)Z)A  - X is in depth 3, Z - in
// depth 1, A in depth zero. Extension _uc means uncompiled. needs to read forward in real time to find the end of the
// expression.
 ...
 *
 * Note: This function is part of a larger regex processing system and is tailored for extended POSIX-style regex
 * patterns.
 *
 * Example Usage:
...
const char *findExpressionEnd_UnionSet_uc(const char *p, const char lp, const char rp)
{
	...
	{
		depth += isValidCommand * (is_int_equal(*p, lp) - is_int_equal(*p, rp));
		depth -= is_int_equal(depth, 1) & is_int_equal(*p, '|'); // '|' affects only on top level. For example:
		                                                         // (XXX(a|B|)DDD) the '|' are not possible alternatives
		isValidCommand = is_int_notEqual(*p, '\\');
	}
	return (depth == 0) ? p : NULL;
}
 ...
 * - Iterates through the pattern to calculate the length of each expression, considering special cases like nested
 * parentheses.
 ...
 * - The current implementation has a complexity that may reach O(n^2) in the worst case, with potential optimization
 * noted.
 *
 * Note: This function is part of a larger regular expression matching system and is intended for internal use within
 * this system.
 *
 * Example Usage:
...
const char *tCompiledRegex::compile(const char *pattern)
{
	...
	// For each 'OPEN' rules calculate the length of the expression. Todo: make it O(n) instead of O(n^2) for worst case
	// of "((((((((((A))))))))))"
	for (int i = 0; *pattern; pattern++, i++)
		exprLen[i] = (UCHAR)(goToNextPat_uc(pattern) - pattern); // 'i' is alwyas equals to (pattern - start).
		                                                         // Initialize the length of current
	end = pattern;
	...
	for (int i = 0; *pattern; pattern++, i++)
	{
		...
		else if (unionLen[i] == 0)
		{ // If we already calculated the length for current union, skip it.
			// We are by definition at the first union. Example: For (AA|BB|CC), We are at |BB|CC).
			int open;
			for (open = i - 1; start + open + exprLen[open] <= pattern; open--)
				; // Go backwards until we find the '(' of the current union.
			// OK, now start+open points exactly to the '(' that opened a union. Moreover 'pattern' points to the first
			// '|'
			const char *next = pattern,
					   *cur = start + open; // Iterate over all the '|' and for each store the length until the next '|'
			while (*next == '|')
			{
			...
			unionLen[cur - start] =
				(UCHAR)(next - cur + 1); // MArk for the last '|' the length until the terminating ')'
		}
	}
	return end;
}

static const tCompiledRegex *compiledRegexPtr =
	NULL; // Pointer to the current regex-used. This line ruins multi-threading capabilities. Feel free to change the
          // system architecture to support mutli-threading
...
int c_achar(const char *pattern, const char *sample) { return (*pattern == *sample) ? 1 : NO_MATCH; }
 ...
 *
 * @return int The number of characters consumed from 'sample' if a successful match is found for any of the
 * alternatives, otherwise returns NO_MATCH.
 *
 * The function operates as follows:
...
int c_group(const char *pattern, const char *sample)
{
	...
	if (!close)
		return NO_MATCH; // Could not match the paretheses. Wrong expresion. Exit
	int nCharsMatched;
	while (close[-1] == '|')
	{
 ...
 *         or returns NO_MATCH in case of failure. If the abbreviation is unknown, it falls back to character
 * comparison.
 *
 * The function operates as follows:
...
#define ABB_LENGTH (32)
	...
	{
		case 'd':
			strcpy_s(abbr, ABB_LENGTH, "[0-9]");
			break; // Digit
		case 'D':
			strcpy_s(abbr, ABB_LENGTH, "[^0-9]");
			break; // Non-digit
		case 'x':
			strcpy_s(abbr, ABB_LENGTH, "[0-9A-Fa-f]");
			break; // Hex digit
		case 'X':
			strcpy_s(abbr, ABB_LENGTH, "[^0-9A-Fa-f]");
			break; // Non Hex
		case 'w':
			strcpy_s(abbr, ABB_LENGTH, "[0-9A-Za-z_]");
			break; // Word character
		case 'W':
			strcpy_s(abbr, ABB_LENGTH, "[^0-9A-Za-z_]");
			break;
		case 'h':
			strcpy_s(abbr, ABB_LENGTH, "[0-9A-Za-z]");
			break; // head of word character
		case 'H':
			strcpy_s(abbr, ABB_LENGTH, "[^0-9A-Za-z]");
			break;
		case 'a':
			strcpy_s(abbr, ABB_LENGTH, "[A-Za-z]");
			break; // Alphabetic character
		case 'A':
			strcpy_s(abbr, ABB_LENGTH, "[^A-Za-z]");
			break;
		case 'l':
			strcpy_s(abbr, ABB_LENGTH, "[a-z]");
			break; // Lowercase character
		case 'L':
			strcpy_s(abbr, ABB_LENGTH, "[^a-z]");
			break;
		case 'u':
			strcpy_s(abbr, ABB_LENGTH, "[A-Z]");
			break; // Uppercase character
		case 'U':
			strcpy_s(abbr, ABB_LENGTH, "[^A-Z]");
			break;
		case 's':
			strcpy_s(abbr, ABB_LENGTH, "[ \t\r\n\v\f]");
	...
	if (*abbr)
		return match(abbr, sample, end_of_string(abbr));
	else
		return c_achar(pattern, sample); // Unknown abbreviation. Just assume that it is a character comparison
}
...
int c_option(const char *pattern, const char *sample)
{
	...
	pattern++;
	close--; // +1 and -1 remove the parentheses
	int negationOp =
		((*pattern == '^') ? NO_MATCH
	                       : 1); // Check for negation flag. Invert character [^a-z], representing negation operator
	if (negationOp < 0)
		pattern++;
	...
	{
		if (*pattern == '-' && from)
		{                     // Check for range selection. Like 0-9, where we already have the from
			to = pattern + 1; // Find the 'to'
			if (*to == '\\')
				to++; // Comparison with reserved character. like \-  or \*
				      // Test for range
		...
		{
			from++;
			pattern++; // Comparison with reserved character. like \\* or \\?
		}
 ...
 *
 * Note: This function is a part of a larger regex matching system and relies on other components like
 * 'compiledRegexPtr'.
 *
 * Example Usage:
...
int c_multi(const char *pattern, const char *sample, const char *endpattern)
{
	...
	switch (*multi)
	{
		case '{': // For range of repetition: {4} or {4-8}
		{
			...
			{
				if (comma < rEnd - 1)
					max = MAX(ft_fast_atoi(comma + 1), 1); // Read Max, Max must be at least 1;
				else if (comma == rEnd - 1)
					max = (1 << 30); // Max does not exists: '{min,}', assume 1 billion is enough. Can use MAX_INT
					                 // instead. Daniel did not want a dependency on <limits.h>
			}
			else
				max = MAX(min, 1); // No range: Like {4}, Max must be at least 1;
		}
		break;
		case '+':
			min = 1;
	...
	if ((min == 0) && (*next_pat))
	{
		...
		if (nRestCharsMatched >= 0)
			foundMatchAt =
				start_sam + nRestCharsMatched; // Yes! The rest of the sample string matches the rest of the pattern.
			                                   // But maybe we can do more repititions and still be fine. like '.*b'
			                                   // matched to 'ab' but can also match  'abccqqb'
		// Note: if nRestCharsMatched==0 than the rest of the pattern can be matched to an empty string. Success is
		// guaranteed. Now we want to match as much repititions as we can. Like 'a*b?' was matched to first character of
		// 'aaaz' but can be matched to 'aaa'.
	}
	...
	while (sample < ends)
	{
		...
		if (nCharsMatched < 0)
		{
			...
			if (nRepitions < min)
				return NO_MATCH; // We need at least 'min' but failed

			if (*next_pat)
			{
				// If (nRestCharsMatched < 0)
				//      We have enough iterations but we already know that the rest of pattern can't be matched. If we
				//      found a good solution earlier return it. Otherwise no solution for matching
				// Else We have found a good solution right now and no more iterations possible. Return the good
				// solution.
		...
		if (nRepitions < min)
			continue; // If we still havent reached the minimal amount of repititions than continue to gather more
			          // repetitions.
		...
		if (*next_pat)
		{
			nRestCharsMatched = match(next_pat, sample, endpattern);
			if (nRestCharsMatched >= 0)
				foundMatchAt = sample + nRestCharsMatched; // See explanation of the code line 'foundMatchAt = start_sam
				                                           // + nRestCharsMatched;' above
		}

		if (nRepitions == max)
		{ // Check the maximal limit of repititions.
			if (*next_pat)
				return (foundMatchAt) ? (int)(foundMatchAt - start_sam)
				                      : NO_MATCH; // See explanation for this exact code line above
	...
	if (nRepitions < min)
		return NO_MATCH; // Sample string terminated and we didn't get our minimal amount.

	if ((*next_pat) && (nRestCharsMatched < 0))
		return (foundMatchAt)
		         ? (int)(foundMatchAt - start_sam)
		         : NO_MATCH; // Sample string terminated and the rest of the pattern cannot be matched to an empty
		                     // string. If we found a good solution return it. Otherwise no solution for matching
	return (int)(sample - start_sam); // No following patterns that require aditional characters and we got enough
	                                  // iterations. like a*z? on a string of 'aaaa'
}
...
static int match(const char *pattern, const char *sample, const char *endpattern)
{
	...
	{
		next_pat =
			compiledRegexPtr->getExpressionEnd(pattern); // Find next pattern to see if it is a suffix like *, {x,y},?,+
		if (next_pat == NULL)
			return NO_MATCH;      // Wrong regular expression. For example '(A)))'
		cmd = get_cmd(*next_pat); // Check the next command if it is a suffix
		if (isSuffix(cmd))
		{ // 'cmd' is indeed a suffix. like 'z{3,7}'. Activate {3,7} on pattern 'z'.
		...
		{ // No suffix
			cmd = get_cmd(*pattern);
			// if (cmd->attr&TYPE_RECURSION){ To do: Handle the the case of the bug (.*)AB is not matched to 'ZAB'
			// because .* consumes 3 letters
...
const char *tCompiledRegex::search(const char *sampleString, int *resLen) const
{
	...
	const char *endOfSearch = EOS =
		end_of_string(sampleString); // When comparint the pattern to sample string we will search the entire sample.
	if (pattern == endPattern)
	{ // Empty pattern is matched with zero length matching
		*resLen = 0;
		return sampleString;
	}

	if (pattern[0] == '^')
	{                                   // Check if first characters forces a matching to beggining of the string
		pattern++;                      // Skip the '^'
	...
	*resLen = match(
		pattern,
		sampleString,
		endPattern); // Try to match pattern to empty string (since we didn't allow it before).
	return (*resLen >= 0)
	         ? sampleString
	         : NULL; // Empty sample string was matched to pattern (like a*b*c*) (if==0). Otherwise no match
};
