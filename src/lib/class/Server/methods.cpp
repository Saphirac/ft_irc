...
{
	std::string str = "123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz+-*/!~@#$%^&()_+{}[]|;<>,.?/";
	std::string newstr;
	int         pos;
...
void Server::check_time_of_last_msg()
{
	     ...
	     ++client_by_socket)
	{
		...
		if (client.get_has_been_pinged() == false && client.time_since_last_msg() / CLOCKS_PER_SEC > TIMEOUT)
		{
